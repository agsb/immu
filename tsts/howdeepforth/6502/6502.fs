 ...
 : < DOCOL - 0< SEMIS ;
 : > DOCOL SWAP < SEMIS ;
 : ROT DOCOL TOR SWAP RFROM SWAP SEMIS ;
 ...
 : COMPILE DOCOL ?COMP RFROM DUP 2+ TOR FETCH COMMA SEMIS ;
 ...
 : (;CODE) RFROM DOCOL LATEST PFA CFA STORE SEMIS ;
 : ;CODE DOCOL ?CSP COMPILE (;CODE) [COMPILE] [ SMUDGE SEMIS ;
 : <BUILDS DOCOL 0 CONSTANT SEMIS ;
 : DOES> DOCOL RFROM LATEST PFA STORE SEMIS ;
 : DODOE ;
 ...
 : (.") R DOCOL COUNT DUP 1+ RFROM + TOR TYPE SEMIS ;
 : ." DOCOL 22 STATE FETCH IF COMPILE (.") WORD HERE C@ 1+ ALLOT ELSE WORD HERE COUNT TYPE ENDIF SEMIS ;
 : EXPECT DOCOL OVER + OVER DO KEY DUP 0E +ORIGIN FETCH = IF DROP 08 OVER I = DUP RFROM 2 - + TOR - ELSE DUP 0D = IF LEAVE DROP BL 0 ELSE DUP ENDIF I C! 0 I 1+ STORE ENDIF EMIT LOOP DROP SEMIS ;
 : QUERY DOCOL TIB FETCH 50 EXPECT 0 IN STORE SEMIS ;
 : X DOCOL BLK FETCH IF 1 BLK +! 0 IN STORE BLK FETCH 7 AND 0= IF ?EXEC RFROM DROP ENDIF ELSE RFROM DROP ENDIF SEMIS ;
 : FILL DOCOL SWAP TOR OVER C! DUP 1+ RFROM 1 - CMOVE SEMIS ;
 : ERASE DOCOL 0 FILL SEMIS ;
 : BLANKS DOCOL BL FILL SEMIS ;
 : HOLD DOCOL -1 HLD +! HLD FETCH C! SEMIS ;
 : PAD DOCOL HERE 44 + SEMIS ;
 : WORD DOCOL BLK FETCH IF BLK FETCH BLOCK ELSE TIB FETCH ENDIF IN FETCH + SWAP ENCLOSE HERE 22 BLANKS IN +! OVER - TOR R HERE C! + HERE 1+ RFROM CMOVE SEMIS ;
 : (NUMBER) BEGIN DOCOL 1+ DUP TOR C@ BASE FETCH DIGIT WHILE SWAP BASE FETCH U* DROP ROT BASE FETCH U* D+ DPL FETCH 1+ IF 1 DPL +! ENDIF RFROM REPEAT RFROM SEMIS ;
 : NUMBER DOCOL 0 0 ROT DUP 1+ C@ 2D = DUP TOR + -1 BEGIN DPL STORE (NUMBER) DUP C@ BL - WHILE DUP C@ 2E - 0 ?ERROR 0 REPEAT DROP RFROM IF DMINUS ENDIF SEMIS ;
 ...
 : M* DOCOL OVER OVER XOR TOR ABS SWAP ABS U* RFROM D+- SEMIS ;
 : M/ DOCOL OVER TOR TOR DABS R ABS U/ RFROM R XOR +- SWAP RFROM +- SWAP SEMIS ;
 : * DOCOL U* DROP SEMIS ;
 : /MOD DOCOL TOR S->D RFROM M/ SEMIS ;
 : / DOCOL /MOD SWAP DROP SEMIS ;
 : MOD DOCOL /MOD DROP SEMIS ;
 : */MOD DOCOL TOR M* RFROM M/ SEMIS ;
 : */ DOCOL */MOD SWAP DROP SEMIS ;
 : M/MOD DOCOL TOR 0 R U/ RFROM SWAP TOR U/ RFROM SEMIS ;
 ...
 : BUFFER DOCOL USE FETCH DUP TOR BEGIN +BUF UNTIL USE STORE R FETCH 0< IF R 2+ R FETCH 7FFF AND 0 R/W ENDIF R STORE R PREV STORE RFROM 2+ SEMIS ;
 : BLOCK DOCOL OFFSET FETCH + TOR PREV FETCH DUP FETCH R - DUP + IF BEGIN +BUF 0= IF DROP R BUFFER DUP R 1 R/W 2 - ENDIF DUP FETCH R - DUP + 0= UNTIL DUP PREV STORE ENDIF RFROM DROP 2+ SEMIS ;
 : (LINE) TOR DOCOL C/L B/BUF */MOD RFROM B/SCR * + BLOCK + C/L SEMIS ;
 : .LINE DOCOL (LINE) -TRAILING TYPE SEMIS ;
 : MESSAGE DOCOL WARNING FETCH IF -DUP IF 4 OFFSET FETCH B/SCR / - .LINE ENDIF ELSE ." MSG # " DOT ENDIF SEMIS ;
 : LOAD DOCOL BLK FETCH TOR IN FETCH TOR 0 IN STORE B/SCR * BLK STORE INTERPRET RFROM IN STORE RFROM BLK STORE SEMIS ;
 ...
 : D/CHAR ;
 : ?DISC DOCOL 1 D/CHAR TOR 0= IF R 15 = IF CR BEGIN 4 D/CHAR EMIT UNTIL QUIT ENDIF BEGIN 4 D/CHAR DROP UNTIL ENDIF RFROM DROP SEMIS ;
 : BLOCK-WRITE ;
 : BLOCK-READ ;
 ...
 : FORGET DOCOL [COMPILE] TICK NFA DUP FENCE FETCH U< 15 ?ERROR TOR VOC-LINK FETCH BEGIN R OVER U< WHILE [COMPILE] FORTH DEFINITIONS FETCH DUP VOC-LINK STORE REPEAT BEGIN DUP 4 - BEGIN PFA LFA FETCH DUP R U< UNTIL OVER 2 - STORE FETCH -DUP 0= UNTIL RFROM DP STORE SEMIS ;
 : TICK DOCOL -FIND 0= 0 ?ERROR DROP [COMPILE] LITERAL SEMIS ;
 ...
 : REPEAT DOCOL TOR TOR [COMPILE] AGAIN RFROM RFROM 2 - [COMPILE] ENDIF SEMIS ;
 ...
 : D.R DOCOL TOR SWAP OVER DABS <# #S SIGN #> RFROM OVER - SPACES TYPE SEMIS ;
 : D. DOCOL 0 D.R SPACE SEMIS ;
 : .R DOCOL TOR S->D RFROM D.R SEMIS ;
